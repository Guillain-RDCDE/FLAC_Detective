...
Scope. This tests the AAC-family hypothesis (2048-sample long block). MP3,
Opus/CELT and Vorbis do not match it and score at the null — which is correct
behaviour, not a gap, since the existing rules already convict on those.
"""
...
from ..mdct import alignment_stat

logger = logging.getLogger(__name__)

# Calibration — measured, not guessed. 880 certified-genuine files (EAC/XLD/
...
#
# Not one genuine file in 880 reached 1.5, while ffmpeg AAC sits an order of
# magnitude above it at every bitrate. Both thresholds therefore sit in empty
# space rather than being squeezed against a tail: 2.0 is 34 % clear of the
# highest genuine file ever measured, 3.0 is double it.
#
...
def apply_rule_13_mdct_alignment(
    ...
    """
    details: dict = {"mdct_peak_ratio": float("nan"), "mdct_offset": -1}
    ...
    try:
        ...
        ratio, offset = alignment_stat(mono, int(sample_rate))
    ...
    logger.info("RULE 13: MDCT peak ratio %.2f at offset %d", ratio, offset)

    if ratio >= RATIO_HARD:
        return (
            SCORE_HARD,
            [
                f"R13: MDCT quantisation grid detected — hole density {ratio:.1f}x higher "
                f"at frame alignment {offset} than at any other (+{SCORE_HARD}pts)"
            ],
            details,
        )
